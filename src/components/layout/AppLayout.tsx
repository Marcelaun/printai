...
const AppLayout: React.FC = () => {
  ...
  const handleProfileComplete = (name: string, cpf: string, isAdmin?: boolean) => {
    const profile = { name, cpf, isAdmin: !!isAdmin };
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (isDraggingSidebar) {
        // O menu master preto da esquerda tem 64px (w-16). Subtraímos para o cálculo ficar exato.
    ...
    <div className="h-screen w-full flex flex-col bg-slate-50 overflow-hidden font-sans">
      <header className="bg-indigo-900 text-white p-4 shadow-md z-20 flex justify-between items-center shrink-0">
        <div className="flex items-center gap-3">
          <div className="bg-indigo-700 p-2 rounded-lg shadow-inner"><Printer size={24} className="text-white" /></div>
          <div>
            ...
            <p className="text-[10px] text-indigo-300 font-bold uppercase tracking-widest opacity-80">Gestão Gráfica Inteligente</p>
          </div>
        </div>
      ...
      <main className="flex-1 flex overflow-hidden relative">
        {loading ? (
          <div className="flex-1 flex items-center justify-center bg-slate-50">
            <div className="flex flex-col items-center gap-4"><Loader2 size={48} className="text-indigo-600 animate-spin" /><p className="text-slate-500 font-bold uppercase tracking-widest text-xs animate-pulse">Carregando Leads...</p></div>
          </div>
        ) : (
          <>
